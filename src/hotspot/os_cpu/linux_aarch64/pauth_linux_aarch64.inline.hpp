...
//
inline address pauth_sign_return_address(address ret_addr, uint64_t modifier) {
...
//
inline address pauth_authenticate_return_address(address ret_addr, uint64_t modifier) {
  if (VM_Version::use_rop_protection()) {
    register address bak = ret_addr;
    register address r17 __asm("r17") = ret_addr;
    register uint64_t r16 __asm("r16") = modifier;
